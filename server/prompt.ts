...
export const buildCodexPrompt = (
  ...
  const cloneGrounding = job.mode === 'clone'
    ...
    : '';
  const sourceEvidenceHardGate = `演示证据硬门禁：旁白提到软件演示、生成结果、网页/视频效果、前后对比、交互、实机操作或可见质量判断时，原片对应画面必须成为该 cue 的主视觉，不能被抽象卡片、图标、粒子或文字总结替代。motion 不是唯一资格；“画面长什么样、功能是否存在、两边差异如何”本身就需要真实证据。每个 visualType=source_video_pip 的 cue 必须写 sourceVideoEvidence，包含 evidenceSubject、归一化 sourceContentBounds、hasBakedSubtitles、hasSourceWatermark、检测到旧字时的 sourceSubtitleBounds/sourceWatermarkBounds，以及 cleanupStrategy=clean-interval|crop|native-rebuild。浏览器/剪辑器录屏裁到真正的生成结果或操作区域；清理标签栏、时间线、旧人物、旧字幕和水印，但不得连演示一起裁掉。没有烧录字幕、水印或旧人物时，sourceContentBounds 默认必须是 [0,0,1,1]，以 contain/完整语义画面方式显示；竖屏剩余区域用纯色或渐变延展，禁止用 cover、水平平移或轻微放大吃掉网页左右边缘。只有审查帧能证明裁切没有丢失任何被讲解的控件、文字、图表或结果时才允许缩小 bounds。演示段允许人物缩小、移到证据之外或完全隐藏；“人物为主”只适用于解释段。scene_implementation 原样复制 sourceVideoEvidence，并在真实容器绑定 data-source-evidence-layer="source-video-pip"、data-source-content-bounds、data-source-cleanup-strategy、data-source-evidence-subject。每个证据 cue 导出一张实际 Remotion 审查帧，确认内容在手机尺寸可看懂。`;
