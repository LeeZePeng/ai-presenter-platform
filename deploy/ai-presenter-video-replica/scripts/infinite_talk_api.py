...
def submit(args: argparse.Namespace) -> dict:
    ...
    for index in range(args.max_polls):
        ...
        if "任务执行完毕" in summary or "成功下载视频" in summary:
            if prompt_id:
                result = completed_result(args, prompt_id, out_dir, start)
                pending_path.unlink(missing_ok=True)
                return result
            break
    if last_result:
        (out_dir / "last_status.json").write_text(json.dumps(last_result, ensure_ascii=False, indent=2))
    result = {
        "provider": "InfiniteTalk",
        "prompt_id": prompt_id,
        "saved": [],
