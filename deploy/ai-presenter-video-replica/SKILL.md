...
3. **Build narration and captions**
   ...
   - For Douyin/WeChat short-form Chinese narration generated by the platform, write and synthesize for a natural brisk delivery: roughly 5.8-7.2 meaningful spoken units per second, short clauses, and about 0.15-0.35 seconds between sentences. Remove filler, duplicated setup, and formal transitions before TTS. For Qwen reference-voice cloning, start at speed 1.12 and only step up to 1.18 after measuring a real candidate; never begin at 1.4-1.5 because it damages timbre and clarity. Generate a candidate WAV, then run `python3 scripts/validate_narration_pace.py --script out/audio/final_script.txt --audio <candidate.wav> --report out/analysis/narration_pace.json --min-rate 5.8 --max-rate 7.2`; only rename/hash it as `final_narration.wav` after this passes. The validator counts Han characters plus spoken Latin/digit tokens against the real ffprobe duration. Never speed up a locked narration with time stretching. Exact replica mode may follow the source cadence when fidelity matters more than short-form pace.
   ...
   - Before paid lip-sync, listen to or inspect the first 15 seconds and confirm the candidate passed `narration_pace.json`. If a short-form read sounds slow or the measured rate is below 5.8, regenerate TTS with tighter punctuation and a faster provider setting; do not compensate with more visual cards or by accelerating the final WAV afterward.
